...
- Live pages are only `/setup` and `/host` (no `/display` route in current code).
...
- WebSocket message types consumed by frontend: `controls_updated`, `events_snapshot`, `event_received`, `game_config_updated`, `teams_updated`.
