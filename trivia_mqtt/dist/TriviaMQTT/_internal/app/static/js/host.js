...
function renderControls() {
  ...
  controls.forEach((control) => {
    ...
    statusEl.textContent = status === "online" ? "En línea" : status === "offline" ? "Desconectado" : "Esperando";
    ...
    node.querySelector(".control-last-seen").textContent = `Última conexión: ${formatDate(control.last_seen)}`;
...
function renderQuestionOptions(question) {
  ...
  const status = String(gameState?.game_status || "").toLowerCase();
  const shouldHighlightCorrect =
    answerRevealed || status === "question_active" || status === "waiting_for_answer";
  const correctAnswer = String(question.correct_answer || "").trim().toUpperCase();

  options.forEach(([letter, text]) => {
    const item = document.createElement("article");
    item.className = "host-option-card neutral";
    const isCorrectByReveal = answerRevealed && revealedAnswer === letter;
    const isCorrectByCurrentQuestion = shouldHighlightCorrect && correctAnswer === letter;
    if (isCorrectByReveal || isCorrectByCurrentQuestion) {
...
function renderGameState() {
  ...
  const showAnswerInQuestionCard =
    Boolean(gameState.current_question) &&
    (gameState.answer_revealed || status === "question_active" || status === "waiting_for_answer");

  if (showAnswerInQuestionCard) {
...
function stateDescription(status) {
  ...
  if (status === "waiting_for_answer") return "Esperando validación";
...
async function callGameAction(endpoint) {
  ...
  if (!response.ok || data.success === false) {
    setActionMessage(data.detail || data.message || "No se pudo ejecutar la acción.", "error");
    return null;
  }
  setActionMessage(data.message || "Acción ejecutada correctamente.", "success");
  return data;
}
