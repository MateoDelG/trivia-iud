...
function renderSetupSummary() {
  ...
  if (setupStatControlsMetaEl) setupStatControlsMetaEl.textContent = `En línea: ${onlineControls}`;
...
function renderDetectedControls() {
  ...
  detectedControlsEl.innerHTML = controls
    .map((control) => {
      ...
      return `<article class="detected-control-chip ${onlineClass}"><div class="detected-control-main"><span class="detected-control-id">${control.device_id}</span><span class="detected-control-sub">${status === "online" ? "En línea" : "Desconectado"}</span></div>${assigned ? `<span class="detected-control-assigned">Asignado a: ${assigned}</span>` : ""}</article>`;
    })
    .join("");
  renderSetupSummary();
}
...
function renderTeamRows() {
  ...
  for (let index = 0; index < totalTeams; index += 1) {
    ...
    if (selectedControl) {
      ...
      statusBadge.textContent = isOnline ? "Control en línea" : "Control desconectado";
...
async function saveGameConfig() {
  ...
  if (!response.ok) {
    ...
    setMessage(data.detail || "No se pudo guardar la configuración.", "error");
    return;
  }
